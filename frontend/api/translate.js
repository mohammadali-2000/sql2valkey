...
// Safely initialize Valkey/Redis connection
let valkey = null;
if (process.env.VALKEY_URL) {
  ...
  valkey.on('error', (err) => {
    ...
    valkey = null; // Disable cache on error to prevent crashes
  });
} else {
  console.log('No VALKEY_URL provided. Cache is disabled. Translating dynamically.');
}
...
app.post('/api/translate', async (req, res) => {
  try {
    ...
    // 1. Check Valkey Cache (only if valkey is connected)
    if (valkey) {
      try {
        ...
        // Continue to rule engine on cache failure
      }
    }
